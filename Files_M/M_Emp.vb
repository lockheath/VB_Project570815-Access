...
Public Class M_Emp
    ...
    Private Sub ShowPosition()
        CurrentRecord = Me.BindingContext(ds, "Mtbl_E").Position + 1
        RecordCount = Me.BindingContext(ds, "Mtbl_E").Count
    ...
    Private Sub M_Emp_Load(sender As Object, e As EventArgs) Handles Me.Load
        txtSearch.Focus()
        ...
        Dim sqlMD As String = ""
        sqlMD = "SELECT * FROM Mtbl_E ORDER BY EID"
        ...
        If txtSearch.Text <> "" Then
            If (e.KeyCode = Keys.Enter) Then 'IF Key.ENTER shall >>>
                ...
                If cboS.SelectedIndex = 0 Then
                    sqlMD &= " WHERE (EID LIKE'%" & txtSearch.Text & "%')"
                ElseIf cboS.SelectedIndex = 1 Then
                    sqlMD &= " WHERE (ENAME LIKE'%" & txtSearch.Text & "%')"
                ElseIf cboS.SelectedIndex = 2 Then
                    sqlMD &= " WHERE (ELNAME LIKE'%" & txtSearch.Text & "%')"
                ElseIf cboS.SelectedIndex = 3 Then
                    sqlMD &= " WHERE (EPHONE LIKE'%" & txtSearch.Text & "%')"
                End If
                sqlMD &= "ORDER BY EID" 'sortby did
                ...
                If IsFind = True Then 'can find?
                    ds.Tables("Mtbl_E").Clear() 'Clear dataset
                End If


                da.SelectCommand.CommandText = sqlMD
                da.Fill(ds, "Mtbl_E") 'new value go to DA >>> datatable type


                If ds.Tables("Mtbl_E").Rows.Count <> 0 Then 'if record not 0
                    IsFind = True 'founddata
                    ClearAllBinding()

                    txtID.DataBindings.Add("Text", ds, "Mtbl_E.EID")
                    txtAdd.DataBindings.Add("Text", ds, "Mtbl_E.EAAD")
                    txtJunwa.DataBindings.Add("Text", ds, "Mtbl_E.EJUNWA")
                    txtLname.DataBindings.Add("Text", ds, "Mtbl_E.ELNAME")
                    txtName.DataBindings.Add("Text", ds, "Mtbl_E.ENAME")
                    txtPhone.DataBindings.Add("Text", ds, "Mtbl_E.EPHONE")
                    txtPOST.DataBindings.Add("Text", ds, "Mtbl_E.EPOST")
                    PicBox.DataBindings.Add(New Binding("image", ds, "Mtbl_E.EPIC"))
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If (txtID.Text = "") Or (txtName.Text = "") Or (txtPhone.Text = "") Then
            MessageBox.Show("กรุณาใส่ข้อมูล รหัส, ชื่อพนักงาน และเบอร์ติดต่อ ด้วยครับ.", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtID.Focus() 'maybe txtName
            Exit Sub
        End If
        ...
        Select Case ActionFlag
            Case "Add"
                sqlMD = "INSERT INTO Mtbl_E([EID],[ENAME],[ELNAME],[EAAD],[EJUNWA],[EPOST],[EPHONE])"
                sqlMD &= " VALUES('" & txtID.Text & "','" & txtName.Text & "','"
                sqlMD &= "'" & txtAdd.Text & "','" & txtJunwa.Text & "','" & txtPOST.Text & "','" & txtPhone.Text & "')"

            Case "Update"
                sqlMD = "UPDATE INTO Mtbl_E([EID],[ENAME],[ELNAME],[EAAD],[EJUNWA],[EPOST],[EPHONE])"
                sqlMD &= " VALUES('" & txtID.Text & "','" & txtName.Text & "','"
                sqlMD &= "'" & txtAdd.Text & "','" & txtJunwa.Text & "','" & txtPOST.Text & "','" & txtPhone.Text & "')"

                sqlMD &= " WHERE (EID='" & txtID.Text & "')"
        End Select
    ...
    Private Sub btnDel_Click(sender As Object, e As EventArgs) Handles btnDel.Click
        ...
        Dim comMD As OleDbCommand = New OleDbCommand()
        Dim tmpMSG As String = ""
        sqlDel = "Delete from Mtbl_E where EID = '" & txtID.Text & "'"
